...
class YtSpider(scrapy.Spider):
    name = 'yt'
    allowed_domains = ['www.yeitu.com']
    start_urls = ['http://www.yeitu.com/meinv/']

    def parse(self, response):
        category = response.xpath('//div[@class="cover-title"]/span/a/@href').extract()
        for url in category:
    ...
    def parse_album(self, response):
        ...
        links = response.xpath('//li[@class="image-box"]/a/@href').extract()
        for link in links:
